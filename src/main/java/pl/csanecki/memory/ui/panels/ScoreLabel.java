...
public class ScoreLabel extends JLabel implements MillisTimerSubscriber {
    ...
    public void update(Duration passed, MillisRefreshment millisRefreshment) {
        String scoreText = switch (millisRefreshment) {
            case OneThousand -> String.format("Timer: %ds", passed.toSecondsPart());
            case OneHundred -> String.format("Timer: %d.%ds", passed.toSecondsPart(), passed.toMillisPart() / 100);
            case Ten -> String.format("Timer: %d.%02ds", passed.toSecondsPart(), passed.toMillisPart() / 10);
        };
        setText(scoreText);
    }
}
